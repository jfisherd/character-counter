...
## Description
In this lab an interactive input element is made in React to exercise using state variables and handling events. The user can type text into an input and see information about the text such as character count, word count, and estimated reading time in minutes.
...
State variables were made for the current input text and stats properties. When the interactive <input> element's onChange() method is called, a customized function updates each state variable in order and the display re-renders.
<br>

> What considerations did you make when calculating reading time?

Reading time assumes 5 words per second (300 words per minute), convert to minutes and round.
<br>

> How did you ensure the UI remained responsive during rapid text input?

Statistical responses are changed using state variables, which are updated every time a new character is typed. No delays in the rendering or "loading" icons were implemented in this lab.
<br>
